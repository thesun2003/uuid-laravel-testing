...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create($this->getTableName('orders'), function (Blueprint $table) {
            ...
            $table->string('name');
            $table->integer('price');
            $table->foreignUuid('user_id')->references('id')->on($this->getTableName('users'))->onDelete('cascade');
            $table->timestamps();
        });

        Schema::create($this->getTableName('product_organisation'), function (Blueprint $table) {
            $table->foreignUuid('product_id')->references('id')->on($this->getTableName('products'))->onDelete('cascade');
            $table->foreignUuid('organisation_id')->references('id')->on($this->getTableName('organisations'))->onDelete('cascade');
            $table->timestamps();
        });

        Schema::create($this->getTableName('order_product'), function (Blueprint $table) {
            $table->foreignUuid('order_id')->references('id')->on($this->getTableName('orders'))->onDelete('cascade');
            $table->foreignUuid('product_id')->references('id')->on($this->getTableName('products'))->onDelete('cascade');
            $table->timestamps();
        });
    }
